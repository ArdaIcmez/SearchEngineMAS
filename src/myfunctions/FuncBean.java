...
public class FuncBean {
	...
	public FuncBean()
	{
		...
		//myFile = "/home/arda/workspace/SearchEngine/rfds/industrial.owl" ;
	}
	...
	public UserBean doAddHistory(UserBean user,String word) {
		String username = user.getNickname();
		String editQuery = ""+
        		"PREFIX User: <http://www.aicmez.com/user/#> \n" +
        		"INSERT DATA { "+
        		"User:"+username+" User:BrowseHistory \""+word+"\" "+
        		"}";
		FileManager.get().addLocatorClassLoader(Main.class.getClassLoader());
		Model oldModel = FileManager.get().loadModel(this.myFile);
		this.doUpdateRDF(oldModel, editQuery);
		jade.util.leap.List tempList= user.getBrowseHistory();
		tempList.add(word);
		user.setBrowseHistory( tempList );
		
		return user;
	}
	public List<UserBean> doPageResult(String url)
	{
